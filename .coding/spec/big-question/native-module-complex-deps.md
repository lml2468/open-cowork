...
In this repo, `electron-builder.yml` `files:` explicitly lists which
`node_modules` folders ship. That works when a native module has minimal JS
dependencies. But if it depends on 20+ npm packages, you'd have to list them all
(and their transitive deps).

## Solutions

### Option A: Ship the full dependency tree

List the module and all its transitive dependencies under `files:` in
...
files:
  ...
  # ... every transitive dep
```

Find them with `npm ls your-native-module --all`.

**Pros**: Complete solution
**Cons**: Tedious, larger bundle size
...
```typescript
// vite.config.ts (main-process build)
...
```typescript
// src/main/system/optional-feature.ts
...
```typescript
// src/main/system/feature-process.ts
...
```yaml
# electron-builder.yml — only ship the working native module
files:
  - node_modules/better-sqlite3/**/* # Works fine
  # - node_modules/complex-native-module/**/*  # Disabled - see issue #123
```

```typescript
// src/main/system/index.ts
