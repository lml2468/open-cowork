...
In this Electron + React + SQLite app, the layers are:
...
State Layer (Zustand store — src/renderer/store/)
        |
        v
IPC Layer (preload contextBridge; typed ClientEvent / ServerEvent unions
           in src/renderer/types/index.ts, dispatched by handleClientEvent)
        |
        v
Main Process (Electron main — src/main/index.ts, SessionManager, agent-runner)
        |
        v
Database Layer (raw better-sqlite3 — src/main/db/database.ts; no ORM)
```
