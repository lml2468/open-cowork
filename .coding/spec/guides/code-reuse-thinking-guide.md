...
### Pattern 5: Data Access via the Zustand Store

This repo has **no React Query / TanStack Query**. The renderer talks to the
main process through the typed `ClientEvent`/`ServerEvent` IPC contract, and
shared state lives in a Zustand store (`src/renderer/store/`). Reuse the store's
actions/selectors instead of re-implementing IPC calls per component.

```typescript
// renderer components read via selectors and dispatch via store actions,
// which in turn call window.electronAPI.send/invoke — see src/renderer/store/index.ts
