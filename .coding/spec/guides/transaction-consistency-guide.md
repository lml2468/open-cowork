...
- [ ] Changed DB binding/serialization of a field
...
### Cause 1: Passing a `Date` instead of a number

**Problem**: better-sqlite3 binds `Date` objects unpredictably; a column meant to
hold Unix ms can end up storing the wrong thing.

```typescript
// Ambiguous — don't bind a Date to an INTEGER column
stmt.run({ created_at: new Date() });
```

**Solution**: Use explicit millisecond integers everywhere:

```typescript
stmt.run({ created_at: Date.now() }); // explicit Unix ms
```

See `../shared/timestamp.md`.
...
- [ ] **DB binding** - is a `Date` being bound where a `number` is expected?
...
3. Bind explicit `Date.now()` numbers to INTEGER columns, never `Date` objects
