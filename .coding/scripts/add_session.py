...
def _auto_commit_workspace(repo_root: Path) -> None:
    """Stage Coding-owned workspace + current-task paths and commit.
    ...
    via ``get_current_task``). We never `git add` the whole `.coding/` tree
    ...
    Honors ``session_auto_commit`` in ``.coding/config.yaml``: when set to
    ...
    # is ``.coding/tasks/<name>`` (or under archive/) — pass the bare name.
    current = get_current_task(repo_root)
    if current:
        task_name = Path(current).name
        paths = safe_coding_paths_to_add(repo_root, task_name=task_name)
    else:
        ...
        paths = [
            p
            for p in safe_coding_paths_to_add(repo_root, task_name=None)
