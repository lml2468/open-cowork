#!/usr/bin/env python3
"""Linear sync hook for Coding task lifecycle.

Syncs task events to Linear via the `linearis` CLI.

Usage (called automatically by task.py hooks):
    python3 .coding/scripts/hooks/linear_sync.py create
    python3 .coding/scripts/hooks/linear_sync.py start
    python3 .coding/scripts/hooks/linear_sync.py archive

Manual usage:
    TASK_JSON_PATH=.coding/tasks/<name>/task.json python3 .coding/scripts/hooks/linear_sync.py sync

Environment:
    TASK_JSON_PATH  - Absolute path to task.json (set by task.py)

Configuration:
    .coding/hooks.local.json  - Local config (gitignored), example:
    {
      "linear": {
        "team": "TEAM_KEY",
...
# Coding priority → Linear priority (1=Urgent, 2=High, 3=Medium, 4=Low)
...
def _load_config() -> dict:
    """Load local hook config from .coding/hooks.local.json."""
    task_json_path = os.environ.get("TASK_JSON_PATH", "")
    if task_json_path:
        # Walk up from task.json to find .coding/
        coding_dir = Path(task_json_path).parent.parent.parent
    else:
        coding_dir = Path(".coding")

    config_path = coding_dir / "hooks.local.json"
    try:
