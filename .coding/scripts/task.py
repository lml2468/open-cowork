...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not full_path.is_dir():
        ...
        print("Hint: Use task name (e.g., 'my-task') or full path (e.g., '.coding/tasks/01-31-my-task')")
        return 1
    ...
    if not resolve_context_key():
        ...
        # Hook didn't inject CODING_CONTEXT_ID (common on Windows + Claude Code,
        ...
        print(colored(
            ...
            "or set CODING_CONTEXT_ID before running task.py start.",
            Colors.YELLOW,
        ))
...
Examples:
  ...
  python3 task.py create "Child task" --slug child --parent .coding/tasks/01-21-parent
  python3 task.py add-context <dir> implement .coding/spec/cli/backend/auth.md "Auth guidelines"
  python3 task.py set-branch <dir> task/add-login
  python3 task.py start .coding/tasks/01-21-add-login
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print("See .coding/workflow.md planning artifact guidance or run:", file=sys.stderr)
        print(
            "  python3 ./.coding/scripts/get_context.py --mode phase --step 1",
            file=sys.stderr,
        )
        print(
