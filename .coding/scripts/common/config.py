#!/usr/bin/env python3
"""
Coding configuration reader.

Reads settings from .coding/config.yaml with sensible defaults.
"""
...
def _strip_inline_comment(value: str) -> str:
    ...
    Mirrors :func:`common.coding_config._strip_inline_comment` so both
...
def get_session_auto_commit(repo_root: Path | None = None) -> bool:
    ...
    Set ``session_auto_commit: false`` in ``.coding/config.yaml`` to skip
...
def get_spec_base(package: str | None = None, repo_root: Path | None = None) -> str:
    """Get the spec directory base path relative to .coding/.
