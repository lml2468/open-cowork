...
_SEED_EXAMPLE = (
    ...
    "Run `python3 .coding/scripts/get_context.py --mode packages` to list available specs. "
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    print("  - Use /coding:continue or phase context to decide the next step", file=sys.stderr)
    print("", file=sys.stderr)
...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    # .coding/tasks/. A mistyped name (e.g. "src") resolves to repo_root/src,
...
def _auto_commit_archive(
    ...
    """Stage Coding-owned task paths and commit after archive.
    ...
    ``git add -f .coding/`` (which would fan out to caches/backups)
    and points users at ``session_auto_commit: false``.

    Honors ``session_auto_commit`` in ``.coding/config.yaml``: when
