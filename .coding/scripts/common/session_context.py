...
_PACKAGE_NAME = "@limenglin/coding"
...
def _collect_package_git_info(
    ...
    When the Coding root is not a Git repo and no configured package repos are
...
def _fetch_coding_version_output() -> str | None:
    try:
...
def _resolve_available_update_version() -> str | None:
    output = _fetch_coding_version_output()
    if not output:
        return None
...
def _get_update_hint(repo_root: Path) -> str | None:
    ...
    return (
        f"Coding update available: {current_version} -> {latest_version}, "
        "run coding update"
    )
