"""
Safe git-add helpers for Coding-owned paths.

Why this module exists
----------------------
A real user incident: a project's `.gitignore` listed `.coding/` (company-wide
...
`git add -f .coding/` — which fan-out-included every ignored subtree
(`.coding/.backup-*/`, `.coding/worktrees/`, `.coding/.template-hashes.json`,
`.coding/.runtime/`), committing 548 files / 83474 lines of caches/backups.

Design
------
- Scripts only stage SPECIFIC product paths (journal files, index.md, the
  current task dir, the archive dir). Never the whole `.coding/` tree.
- If plain `git add <specific>` fails with "ignored by", DO NOT retry with
  ``-f``. The presence of `.coding/` in `.gitignore` is treated as user
  intent ("keep .coding/ local-only"). The script warns and skips the
...
- The warning includes a negative example: ``Do NOT use `git add -f .coding/` ...``
...
# Paths under .coding/ that must NEVER be auto-staged. Listed here so the
# warning to the user can show concrete subpaths to ignore individually
# instead of ignoring the whole `.coding/` tree.
CODING_IGNORED_SUBPATHS = (
    ".coding/.backup-*",
    ".coding/worktrees/",
...
def safe_coding_paths_to_add(
    ...
    Included:
      - .coding/workspace/<developer>/journal-*.md
      - .coding/workspace/<developer>/index.md
      - .coding/tasks/<task_name>/   (ONLY the current task dir when
    ...
    Excluded (intentionally — these must not be staged):
      - .coding/.backup-*, .coding/worktrees/,
        .coding/.template-hashes.json, .coding/.runtime/, .coding/.cache/
...
def safe_archive_paths_to_add(
    ...
    `.coding/tasks/` subtree the old way (active tasks + archive). New
...
def print_gitignore_warning(paths: list[str]) -> None:
    ...
    ``Do NOT use `git add -f .coding/``` — agents reading the warning are
    known to invent that command, which fans out to ignored caches/backups.
    """
    print(
        "[WARN] git add failed because .coding/ paths are ignored by your .gitignore.",
        file=sys.stderr,
    )
    print(
    ...
    print(
        "[WARN] Coding manages these specific paths and they should be tracked:",
        file=sys.stderr,
    )
    if paths:
        for p in paths:
        ...
        print(
            "[WARN]   .coding/workspace/<developer>/{journal-*.md,index.md}",
            file=sys.stderr,
        )
        print(
            "[WARN]   .coding/tasks/<task-dir>/",
            file=sys.stderr,
        )
        print(
            "[WARN]   .coding/tasks/archive/",
            file=sys.stderr,
        )
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Recommended: change your .gitignore from `.coding/` to specific",
        file=sys.stderr,
    )
    print(
    ...
    for sub in CODING_IGNORED_SUBPATHS:
        print(f"[WARN]   {sub}", file=sys.stderr)
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Or, if you intentionally keep .coding/ local-only, set in",
        file=sys.stderr,
    )
    print(
        "[WARN] .coding/config.yaml:",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Do NOT use `git add -f .coding/` — it pulls in backups, worktrees,",
        file=sys.stderr,
    )
    print(
