...
def is_within_tasks_dir(task_dir_abs: Path, repo_root: Path | None = None) -> bool:
    ...
    A real task lives directly at ``.coding/tasks/<name>``. This returns True
...
def resolve_task_dir(target_dir: str, repo_root: Path) -> Path:
    ...
    - Relative path: .coding/tasks/01-31-my-task
    ...
    # Relative path (contains path separator or starts with .coding)
    if "/" in normalized or normalized.startswith(".coding"):
...
if __name__ == "__main__":
    ...
    print(f"is_safe_task_path('.coding/tasks/test'): {is_safe_task_path('.coding/tasks/test', repo)}")
