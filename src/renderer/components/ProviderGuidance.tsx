...
export function GuidanceInlineHint({ text, tone = 'info' }: GuidanceInlineHintProps) {
  ...
  return (
    <div className={`flex items-start gap-2 rounded-xl border px-3 py-2 text-caption ${toneClass}`}>
...
export function CommonProviderSetupsCard({ setups, onApplySetup }: CommonProviderSetupsCardProps) {
  ...
  return (
    <details className="rounded-4xl border border-border-subtle bg-background/40 px-4 py-4">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-body-sm font-medium text-text-primary">
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="min-w-full text-left text-caption text-text-secondary">
          <thead>
              ...
              >
                ...
                <td className="px-2 py-3 text-right">
                  <button
                    ...
                    className="rounded-lg border border-border px-3 py-1.5 text-caption font-medium text-text-primary transition-colors hover:bg-surface-hover"
                  >
