...
describe('SessionState unified store', () => {
  ...
  describe('composer mode', () => {
    it('should default to build mode', () => {
      useAppStore.getState().addSession(makeSession('s1'));
      expect(useAppStore.getState().sessionStates['s1'].mode).toBe('build');
    });

    it('should set the session mode', () => {
      useAppStore.getState().addSession(makeSession('s1'));
      useAppStore.getState().setSessionMode('s1', 'plan');
      expect(useAppStore.getState().sessionStates['s1'].mode).toBe('plan');
    });

    it('should isolate mode per session', () => {
      useAppStore.getState().addSession(makeSession('s1'));
      useAppStore.getState().addSession(makeSession('s2'));
      useAppStore.getState().setSessionMode('s1', 'plan');
      expect(useAppStore.getState().sessionStates['s1'].mode).toBe('plan');
      expect(useAppStore.getState().sessionStates['s2'].mode).toBe('build');
    });
  });
