...
`.coding/workflow.md` is the Coding workflow source of truth inside the user project. An AI does not need Coding source code to understand how the current project should move tasks forward; this file is enough.

## File Responsibilities

`.coding/workflow.md` has three responsibilities:
...
- Platforms with sub-agent support: dispatch `coding-implement` by default for implementation and `coding-check` for checking.
- Platforms without sub-agent support: the main session reads skills such as `coding-before-dev`, then executes directly.
...
After editing, make the AI reread `.coding/workflow.md`; do not assume the flow from the old conversation is still valid.
...
- skills, such as `coding-brainstorm` and `coding-check`.
