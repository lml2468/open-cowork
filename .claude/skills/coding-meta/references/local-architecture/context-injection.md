...
Coding context injection aims to make AI read the right files at the right time instead of relying on model memory. In a user project, injection is implemented by `.coding/` scripts together with platform hooks, agents, and skills.

## Injected Context Types

| Type             | Source                                                                  | Purpose                                                                      |
| ---------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| session context  | `.coding/scripts/get_context.py`                                        | Current developer, git status, active task, active tasks, journal, packages. |
| workflow context | `.coding/workflow.md`                                                   | Current Coding flow and next action.                                         |
| spec context     | `.coding/spec/` + task JSONL                                            | Specs that must be followed during implementation/checking.                  |
| task context     | `.coding/tasks/<task>/prd.md`, `design.md`, `implement.md`, `research/` | Current task requirements, design, execution plan, and research.             |
...
Platforms with session-start support inject a Coding overview when a session starts, clears, compacts, or receives a similar event. Injected content usually includes:
...
Implement and check agents need task context. Coding has two loading modes:
...
```jsonl
{
  "file": ".coding/spec/backend/index.md",
...
Active task state lives in `.coding/.runtime/sessions/` and is isolated per session. Hooks try to resolve the context key from platform events, environment variables, transcript paths, or `CODING_CONTEXT_ID`.
...
| Change per-turn workflow-state rules  | `[workflow-state:STATUS]` block in `.coding/workflow.md`. The platform workflow-state hook parses these blocks verbatim and embeds no fallback text. |
| Change how sub-agents read context    | Platform agent definitions, the `inject-subagent-context` hook, or agent preludes.                                                                   |
| Change JSONL validation/display       | `.coding/scripts/common/task_context.py`.                                                                                                            |
| Change active task resolution         | `.coding/scripts/common/active_task.py`.                                                                                                             |
