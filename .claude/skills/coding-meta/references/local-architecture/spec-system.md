...
`.coding/spec/` is the user's project-specific engineering spec library. Coding is not about making AI memorize conventions; it injects relevant specs or requires the AI to read them at the right time.
...
`.coding/config.yaml` can declare packages:
...
```jsonl
{"file": ".coding/spec/cli/backend/index.md", "reason": "CLI backend conventions"}
{"file": ".coding/spec/cli/unit-test/conventions.md", "reason": "Test expectations"}
```
...
When the AI learns a new rule during implementation or debugging, it should update `.coding/spec/` rather than only summarizing it in chat.

## Local Customization Points

| Need                                              | Edit location                                                                |
| ------------------------------------------------- | ---------------------------------------------------------------------------- |
| Add a new spec layer                              | `.coding/spec/<package>/<layer>/index.md` and corresponding guideline files. |
| Change monorepo spec mapping                      | `packages` / `default_package` / `spec_scope` in `.coding/config.yaml`.      |
...
| Change when specs should be updated               | Phase 3.3 in `.coding/workflow.md` and the `coding-update-spec` skill.       |

## Boundaries

`.coding/spec/` is the user's project specification, not a permanent copy of Coding built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating Coding default templates as immutable documents.
