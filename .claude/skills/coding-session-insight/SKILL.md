...
This skill teaches an AI **how to call `coding mem`** — the project's cross-session memory feedstock — and **when reaching for it is the right move**.
...
A local CLI that indexes the user's past Claude Code conversation logs and lets you list, search, slice by Coding task boundaries, and dump cleaned dialogue from them. Claude Code sessions live under `~/.claude/projects/`.
...
- You are in a sub-agent (`coding-implement` / `coding-check`) whose dispatch prompt already includes the curated `implement.jsonl` / `check.jsonl` context. Adding `mem` on top usually just clutters.
...
- **Update `.coding/spec/`** if the finding is a project-wide convention or gotcha that would help future tasks. Run the `coding-update-spec` skill for that — `session-insight` ends at the discovery.
...
Coding does not prescribe a single destination. Forcing every recall into a fixed file makes the file grow into noise. Let the situation decide.
...
coding mem search "<keyword>"

# Dump dialogue from one session, optionally filtered by phase or keyword.
coding mem extract <session-id> --phase brainstorm
coding mem extract <session-id> --grep "<keyword>"

# Drill into a session: top-N hit turns + surrounding context.
coding mem context <session-id> --turns 3 --around 2

# When you do not know the session id yet, start with list + filter.
coding mem list --cwd <project-path>
coding mem projects   # → list active project cwds, then narrow
```
...
- `mem` is read-only on the Claude Code JSONL store. It does not push or sync to remote.
- This skill does not replace `coding-update-spec` (which is the right tool for promoting a finding into project-wide guidance) or the native task / spec workflow.
