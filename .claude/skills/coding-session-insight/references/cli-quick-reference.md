# `coding mem` CLI Reference

Full flag reference for the five subcommands. Pin this as the authoritative source — `coding mem help` prints the same content at runtime, so anything here that drifts is a bug.
...
| ------------------------------------ | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--platform claude\|all`             | all               | Only Claude Code is indexed, so `claude` and `all` are equivalent.                                                                                        |
...
| `--phase brainstorm\|implement\|all` | extract           | Slice session by Coding task boundaries. `brainstorm` = `[task.py create, task.py start)`. `implement` = turns outside brainstorm windows. Default `all`. |
...
| `--include-children`                 | search / context  | Merge sub-agent sessions into their parent session.                                                                                                       |
...
coding mem search "deadlock" --global --limit 20

# Inside a specific session, surface the top 5 turns that mention "lock contention"
# plus 2 turns of surrounding context.
coding mem context 5842592d --grep "lock contention" --turns 5 --around 2

# Recover the brainstorm window for a session — useful when continuing a task
# the user started a week ago.
coding mem extract 5842592d --phase brainstorm

# List every project this machine has Coding sessions for, with counts.
coding mem projects
```
...
- **`mem` indexes Claude Code JSONL files directly.** If the user has cleared their Claude Code session storage, `mem` cannot recover what is no longer on disk.
- **`mem` is read-only.** No remote sync, no edits to the JSONL store. Any write you do based on `mem` findings is your own follow-up call into the editing tools available to you.

## When you need more than this reference

Run `coding mem help` in the user's shell. The runtime help is authoritative and will be ahead of this reference during fast-moving beta releases.
