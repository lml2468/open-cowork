---
name: coding-spec-bootstrap
description: 'Bootstrap project-specific Coding coding specs with a platform-neutral single-agent workflow. Use when creating or refreshing .coding/spec guidelines, analyzing a codebase with GitNexus, ABCoder, or source inspection, decomposing package/layer spec work, and writing real codebase-backed spec docs without placeholder text.'
---

# Coding Spec Bootstrap

Use this skill to create or refresh `.coding/spec/` guidelines from the real codebase. One capable agent owns the full loop: analyze the repository, choose the spec boundaries, write the docs, and verify the result. The workflow does not depend on a specific host, CLI, or agent brand.

## Workflow

1. Confirm Coding is initialized and inspect the current `.coding/spec/` tree.
...
| Writing high-signal Coding spec files     | [references/spec-writing.md](references/spec-writing.md)               |
...
- Do not leave placeholder text, empty headings, or copied boilerplate in `.coding/spec/`.

## Done Criteria

- `.coding/spec/` describes the project as it exists now.
