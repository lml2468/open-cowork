---
name: coding-update-spec
description: 'Captures executable contracts and coding conventions into .coding/spec/ documents. Use when learning something valuable from debugging, implementing, or discussion that should be preserved for future sessions.'
---
...
```bash
cat .coding/spec/<category>/<file>.md
```
...
```
Development Flow:
  Learn something → /coding:update-spec → Knowledge captured
       ↑                                  ↓
  /coding:break-loop ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- `/coding:break-loop` - Analyzes bugs deeply, often reveals spec updates needed
- `/coding:update-spec` - Actually makes the updates
- `/coding:finish-work` - Reminds you to check if specs need updates
